...
/* Create Enumeration of Possible lval Types also known as Lisp values */
...
struct lval {
  ...
  /* Function - Custom ones and Builtins */
  lbuiltin builtin;
  lenv* env;
  lval* formals;
  lval* body;
...
void lval_del(lval* v) {
  ...
  switch (v->type) {
    ...
    /* For Err, Sym and Str free the string data */
...
lval* builtin_error(lenv* e, lval* a) {
  ...
  /* Constuct Error from first argument */
...
lval* lval_call(lenv* e, lval* f, lval* a) {
  ...
  while (a->count) {
   ...
   if (f->formals->count > 0 && 
      ...
      /* Check to ensure that & is not passed ivalidly. */
...
lval* lval_read_str(mpc_ast_t* t) {
  ...
  /* Copy the string missing out the first quote charachter */
...
if (argc == 1) {
 ...
 while(1) {
    ...
    /* Attempt to Parse the user Input and Evaluate */
    mpc_result_t r;
